...
class smart_cell():
    def __init__(self, row, cell, game_dimensions):
...
def create_smart_grid(game_dimensions=5):
    smart_grid = Dict()
    for dimension in xrange(game_dimensions):
        smart_grid[dimension] = []
    for row in smart_grid:
        smart_grid[row] = [smart_cell(row, cell, game_dimensions) for cell in xrange(game_dimensions)]
    return smart_grid
...
if __name__ == "__main__":
    ...
    new_grid = fill_a_word_to_grid('123456789qwerty', '2-3', grid)
